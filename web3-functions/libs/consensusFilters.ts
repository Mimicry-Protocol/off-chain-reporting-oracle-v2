...
export function mad(values: Array<BigNumber>): Array<BigNumber>
{
    ...
    const mean: BigNumber = values.reduce(
        (acc: BigNumber, val: BigNumber) => acc.add(val), BigNumber.from(0)
    ).div(values.length);
    
    const deviations: BigNumber[] = values.map(num => num.sub(mean).abs());
    
    const meanAbsoluteDeviation: BigNumber = deviations.reduce(
...
export function majority(values: Array<BigNumber>): Array<BigNumber>
{
    ...
    const filteredValues: BigNumber[] = Array.from(mostCommon).map((v) => BigNumber.from(v));
