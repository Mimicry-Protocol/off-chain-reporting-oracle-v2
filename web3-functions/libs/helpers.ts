...
export function unwrapContractPointers(contractsList: string[]): Array<ContractPointer> {
    ...
    const promises: Promise<BigNumber>[] = nftCollections.map(
        throttle(async (nftCollection) => {
            ...
            const nftCollectionInfo: NftCollectionInfo = {
                address, chain, currency, metric
            };
            return await providerClass[providerFunction](
                nftCollectionInfo
            );
    }));
