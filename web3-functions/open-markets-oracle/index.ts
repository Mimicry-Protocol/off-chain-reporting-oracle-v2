...
Web3Function.onRun(async (context: Web3FunctionContext) => {
  try {
    ...
    if (mumbaiDataFeedId === 0 || polygonDataFeedId === 0)
      throw new Error(
    ...
    if (latestOracleValue.gt(0)) {
      ...
      const valueDiffPercent = valueDiff
        .mul(10000)
        .div(latestOracleValue)
        .abs();
      console.log(
        `Value difference since latest update: ${valueDiffPercent.div(100)}%`
      );
      ...
      const timeDiff = BigNumber.from(block.timestamp).sub(
      ...
      if (valueDiffPercent.lte(deviation) && timeDiff.lte(heartbeat)) {
        return {
          canExec: false,
          message: `Value difference is less than ${
            deviation / 100
          }% and the last update was less than ${heartbeat / 60} minutes ago`,
        };
      }
    }
